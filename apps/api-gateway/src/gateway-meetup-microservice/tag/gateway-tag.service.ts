...
export class GatewayTagService {
  ...
  async readAll(readAllTagOptions: IReadAllTagOptions): Promise<ReadAllResult<TagFrontend>> {
    const tags = await firstValueFrom(this.client.send(METADATA.MP_GET_ALL_TAGS, readAllTagOptions));
    return tags;
  }

  async readById(id: string): Promise<TagFrontend> {
    const tag = await firstValueFrom(this.client.send(METADATA.MP_GET_TAG_BY_ID, { id }));
    return tag;
  }

  async create(createTagDto: CreateTagDto): Promise<TagFrontend> {
    const createdTag = await firstValueFrom(this.client.send(METADATA.MP_CREATE_TAG, createTagDto));
    return createdTag;
  }

  async update(id: string, updateTagDto: UpdateTagDto): Promise<TagFrontend> {
    const updatedTag = await firstValueFrom(this.client.send(METADATA.MP_UPDATE_TAG_BY_ID, { id, updateTagDto }));
    return updatedTag;
  }

  async deleteById(id: string): Promise<void> {
    await this.client.emit(METADATA.EP_DELETE_TAG_BY_ID, { id });
  }
}
