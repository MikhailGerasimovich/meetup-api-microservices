...
import { MEETUP_MICROSERVICE, METADATA } from '../../constants/gateway-meetup.constants';
...
export class GatewayMeetupService {
  ...
  async create(createMeetupDto: CreateMeetupDto): Promise<MeetupFrontend> {
    const createdMeetup = await firstValueFrom(this.client.send(METADATA.MP_CREATE_MEETUP, createMeetupDto));
