...
export class MeetupService {
  ...
  async readAll(readAllMeetupOptions: IReadAllMeetupOptions): Promise<ReadAllResult<MeetupType>> {
    const meetups = await firstValueFrom(this.client.send(METADATA.MP_GET_ALL_MEETUPS, readAllMeetupOptions));
    return meetups;
  }

  async readById(id: number): Promise<MeetupType> {
    const meetup = await firstValueFrom(this.client.send(METADATA.MP_GET_MEETUP_BY_ID, { id }));
    return meetup;
  }
  ...
  async create(createMeetupDto: CreateMeetupDto, organizer: JwtPayloadDto): Promise<MeetupType> {
    const createdMeetup = await firstValueFrom(
      this.client.send(METADATA.MP_CREATE_MEETUP, { createMeetupDto, organizer }),
    );
    return createdMeetup;
  }

  async joinToMeetup(meetupId: number, member: JwtPayloadDto): Promise<MeetupType> {
    const meetup = await firstValueFrom(this.client.send(METADATA.MP_JOIN_TO_MEETUP, { meetupId, member }));
    return meetup;
  }

  async leaveFromMeetup(meetupId: number, member: JwtPayloadDto): Promise<MeetupType> {
    const meetup = await firstValueFrom(this.client.send(METADATA.MP_LEAVE_FROM_MEETUP, { meetupId, member }));
    return meetup;
  }

  async update(id: number, updateTagDto: UpdateMeetupDto): Promise<MeetupType> {
    const updatedTag = await firstValueFrom(this.client.send(METADATA.MP_UPDATE_MEETUP_BY_ID, { id, updateTagDto }));
    return updatedTag;
  }

  async deleteById(id: number): Promise<void> {
    await this.client.emit(METADATA.EP_DELETE_MEETUP_BY_ID, { id });
  }
}
